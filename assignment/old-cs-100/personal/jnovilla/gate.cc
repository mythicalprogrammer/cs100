...
Gate::Gate(string typeInput,string nameInput)
{
  gateType=typeInput;
  gateName=nameInput;
  wireZero=NULL;
...
void Gate::gateOutput()//will have wires as left and right and if its an output wire it will just call gateOutput() again
{
//will check the wire types and if theire external then its cool and does its thing but if not
  gateOutput(wireOut);
}

void Gate::gateOutput(Wire *wireOutput)//will have wires as left and right and if its an output wire it will just call gateOutput() again
{
//will check the wire types and if theire external then its cool and does its thing but if not
